...
import { PAGE_SIZE, useAccessLogsReportData } from './useAccessLogsReportData'
...
export default function AccessLogsReportPage() {
  ...
  const { isLoading, error, logs, lookupsError, pagination } = useAccessLogsReportData({
    page,
    status,
    from: from || undefined,
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
        <div className="flex justify-between bg-canvas px-5 py-3 text-xs font-bold uppercase text-muted">
          ...
          <p className="w-[140px] text-center">Portão de Entrada</p>
          <p className="w-[120px] text-center">Entrada</p>
          ...
          logs.map((log) => {
            ...
            return (
              <div key={log.id} className="flex items-center justify-between border-t border-gray-200 px-5 py-3.5">
                ...
                <p className="w-[140px] truncate text-center text-sm text-gray-700">{log.entryGateName}</p>
                <p className="w-[120px] text-center text-sm text-gray-700">{formatDateTime(log.entryTime)}</p>
                <p className="w-[120px] text-center text-sm text-subtle">{formatDateTime(log.exitTime)}</p>
