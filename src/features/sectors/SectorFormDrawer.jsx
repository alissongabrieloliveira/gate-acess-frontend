...
/**
 * Cria ou edita um setor de destino (POST/PUT /sectors). Só admin acessa
 * este drawer (SectorsPage já esconde o botão/ações pra operador comum — o
 * backend também barra 403 se chamado sem permissão). "Ativo" só aparece
 * editando — setor novo já nasce ativo por padrão no banco (mesmo critério
 * já usado em people/vehicles/users/gates).
 */
export default function SectorFormDrawer({ sector, onClose, onSaved }) {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (isEditing) {
        await api.put(`/sectors/${sector.id}`, { ...payload, isActive })
      } else {
      ...
      setError(getErrorMessage(err, 'Não foi possível salvar o setor.'))
    } finally {
  ...
  return (
    <SlideOver
      ...
      subtitle={isEditing ? name : 'Cadastre um novo setor de destino da empresa.'}
      onClose={onClose}
      footer={
        <>
          <button
    ...
    >
      <form id="sector-form" onSubmit={handleSubmit} className="flex flex-col gap-4">
        <div className="flex flex-col gap-3">
          <div className="flex items-center gap-1.5">
            <StepBadge number={1} />
            <p className="text-[13px] font-bold text-ink">Dados do Setor</p>
          </div>
          ...
          <div className="flex flex-col gap-1">
            ...
            <textarea
              ...
              rows={3}
              placeholder="Ex.: Depósito de materiais, acesso restrito a fornecedores"
          ...
          {isEditing && (
            <div className="flex items-center justify-between gap-3 rounded-lg border border-gray-200 p-3">
              <div className="flex flex-col gap-0.5">
                ...
                <p className="text-[11px] text-muted">Setores inativos somem do seletor de destino ao registrar uma entrada.</p>
              </div>
